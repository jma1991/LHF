...
knitr::opts_chunk$set(
  autodep    = TRUE,
  cache      = TRUE,
  cache.path = "cache/03-analysis.Rmd/",
  cache.lazy = FALSE,
  dev        = "png",
  error      = FALSE,
  message    = FALSE,
...
```{r}
mnn <- read_rds(here("data/02-analysis.rds"))
```
...
Plot UMAP showing germ layer and batch annotation:
...
sclust <- function(obj) {
  ...
  dec <- combineVar(modelGeneVar(ref), modelGeneVar(sce, block = sce$phase))
...
```{r}
saveRDS(mnn, file = here("data/03-analysis.rds"))
```
